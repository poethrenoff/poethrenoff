...
- Исправлен сброс фокуса на поле контента при нажатии кнопки «Добавить»: в `openNew()` фокус теперь вызывается unconditionally, а не только при наличии черновика; в `focusNewTextarea()` и `recognizeAudio()` заменён неработающий `document.querySelector('[x-ref="..."]')` на прямой доступ через Alpine `$refs` (`this.$refs[refName].querySelector('.poem-textarea')`), так как Alpine не добавляет `x-ref` в DOM.
