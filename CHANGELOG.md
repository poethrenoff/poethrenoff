...
- Исправлен сброс фокуса на поле контента при нажатии кнопки «Добавить»: в `openNew()` фокус теперь вызывается unconditionally, а не только при наличии черновика; в `focusNewTextarea()` фокус обёрнут в `setTimeout(..., 0)` для стабильной работы после `$nextTick()`, а доступ к элементу осуществляется через Alpine `$refs` (`this.$refs[refName].querySelector('.poem-textarea')`), так как Alpine не добавляет `x-ref` в DOM.
