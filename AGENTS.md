...
- Оптимизированы методы расчета серий стихов в `PoemRepository`:
    ...
    - Оптимизированы SQL-запросы: используется `DISTINCT` и `getSingleColumnResult()` для минимизации объема передаваемых данных.
    - Улучшена надежность: для расчетов используется UTC и `checkdate()`.

- Дашборд мастерской дополнен статистикой «Дней подряд»:
    - В `PoemRepository` добавлен метод `findStreak()`, который считает количество последовательных дней (начиная с сегодняшнего) с написанными стихами. Дата берется из поля `comment` в формате `DD.MM.YYYY`, невалидные значения игнорируются, дубликаты дат уникализируются.
