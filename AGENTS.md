...
4. **Ассеты и загрузки:** В каждой поддиректории `htdocs/` созданы симлинки `bundles` и `assets` на папки в `public/`. Загрузки (uploads) хранятся непосредственно в `htdocs/{context}/upload/`, что позволяет обращаться к медиафайлам по унифицированному пути `/upload/...` независимо от сайта.
 ...
 7. **Версионирование ассетов:** Статические ассеты версионируются по `mtime` через `App\Asset\FileVersionStrategy` (`src/Asset/FileVersionStrategy.php`). К URL CSS/JS добавляется query-параметр `?v=<timestamp>`, предотвращая устаревшее кеширование браузером. Конфигурация в `config/packages/framework.yaml` и `config/services.yaml`.
